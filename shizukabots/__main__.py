...
import config
from shizukabots import LOGGER, app, userbot
from shizukabots.core.call import Aviax
from shizukabots.misc import sudo
from shizukabots.plugins import ALL_MODULES
from shizukabots.utils.database import get_banned_users, get_gbanned
...
async def init():
    ...
    for all_module in ALL_MODULES:
        importlib.import_module("shizukabots.plugins" + all_module)
    LOGGER("shizukabots.plugins").info("Successfully Imported Modules...")
    await userbot.start()
    await Aviax.start()
    try:
        await Aviax.stream_call("https://te.legra.ph/file/29f784eb49d230ab62e9e.mp4")
    except NoActiveGroupCall:
        LOGGER("shizukabots").error(
    ...
    await app.stop()
    await userbot.stop()
    LOGGER("shizukabots").info("Stopping Aviax Music Bot...")
